...
response = requests.get(mat_url)
mat_img = response.content
response = requests.get(Temp_url)
Temp_img = response.content

add(Temp_img, mat_img, 'new_img.jpg', 30, 60) #合成したい画像、合成の背景にしたい画像、出力先、合成する位置の y 座標、x 座標を指定

print('new_img.jpg') #画像の表示(add()の３つ目の引